	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		...
		if (cType != null) {
			if (event.getEntity().getLocation().getWorld().getTime() < 12000
				...
				String data = SpawnMob.config.getString(natureLightNode);
				...
				String data = SpawnMob.config.getString(
						natureNightNode);
				if (Friendly.shouldTarget(event.getEntity(), data, true)) {
					Creature c = (Creature) event.getEntity();
					if (event.getDamager() instanceof LivingEntity) {
		...
		if (cType != null) {
			if (cType == CreatureType.CREEPER) {
				if (event.getEntity().getLocation().getWorld().getTime() < 12000
					...
					String data = SpawnMob.config.getString(natureExplodeDayNode);
					Creature c = (Creature) event.getEntity();
					...
					String data = SpawnMob.config.getString(natureExplodeNightNode);
					Creature c = (Creature) event.getEntity();
					if (Friendly.shouldExplode(event.getEntity(), data, attacked.contains(c)) != true) {
						event.setCancelled(true);
					}
